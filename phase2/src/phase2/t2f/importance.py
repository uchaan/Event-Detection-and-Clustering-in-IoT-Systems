...
from phase2.t2f.utils.PFA import PFA
import warnings
...
def feature_selection(df_feats: pd.DataFrame, labels: dict = None, context: dict = None):
    if labels:
        ...
        df_test_features = pd.concat(
            [df_train_features, df_test_features], axis=0, ignore_index=True
        )
        params = {
        ...
        top_features = features_scoring_selection(
            df_train_features, y_train, mode=score_mode, top_k=top_k, strategy="sk_base"
        )
    else:
        top_features = features_scoring_selection(
...
def features_scoring_selection(
    df: pd.DataFrame, labels: list, mode: str = "simple", top_k: int = 20, strategy: str = "multi"
):
    ...
    if strategy == "tsfresh":
        relevance_features = relevance.calculate_relevance_table(
            df, labels, ml_task="classification", multiclass=False
        )
        ...
        relevance_features = relevance.calculate_relevance_table(
            df, labels, ml_task="classification", multiclass=True
        )
...
def simple_grid_search(
    df_base: pd.DataFrame, y_base: list, df_complete: pd.DataFrame, params: dict
):
    ...
    for new_params in tqdm(ParameterGrid(grid_params)):
        if not k_best:
            ...
            top_features = features_scoring_selection(
                df_base.iloc[:max_rows, :],
                y_base[:max_rows],
                mode=new_params["score_mode"],
                top_k=new_params["top_k"],
                strategy=strategy,
            )
        else:
