...
def pot_th(train_anomaly_score, anomaly_score, q=1e-3, level=0.99, dynamic=False):
    ...
    logging.info(
        "[POT] Test score max: {}, min: {}".format(anomaly_score.max(), anomaly_score.min())
    )
    ...
    if pot_th is None:
        ...
        logging.info(
            "POT cannot find the threshold, use {}% percentile {}".format(level[0] * 100, pot_th)
        )
    return pot_th
...
def best_th(
    ...
    for anomaly_percent in search_range:
        ...
        # print("label shape=", anomaly_label.shape)

        if pred.shape != anomaly_label.shape:
            pred = pred.T
...
def custom_best_th(
    ...
    logging.info("Searching for the best threshod.. (custom ver.)")
    thresholds = np.arange(0.0, np.max(anomaly_score), np.max(anomaly_score) / 100)
    search_history = []
    if point_adjustment:
        target_metric = target_metric + "_adjusted"
    ...
    for index, threshold in enumerate(thresholds):
        ...
        logging.debug(f"th={threshold}, {target_metric}={current_value}")
